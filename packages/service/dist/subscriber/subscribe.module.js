"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SubscribeModule = void 0;
const common_1 = require("@nestjs/common");
const schedule_1 = require("@nestjs/schedule");
const contractEventSubscribe_service_1 = require("./contractEventSubscribe.service");
const ether_provider_1 = require("../lib/ether.provider");
const order_service_1 = require("../order/order.service");
const order_module_1 = require("../order/order.module");
const mongoose_1 = require("@nestjs/mongoose");
const order_schema_1 = require("../order/schema/order.schema");
const seaport_provider_1 = require("../lib/seaport.provider");
const block_schema_1 = require("../block/schema/block.schema");
const block_service_1 = require("../block/block.service");
const MutexManager_1 = require("./MutexManager");
let SubscribeModule = exports.SubscribeModule = class SubscribeModule {
};
exports.SubscribeModule = SubscribeModule = __decorate([
    (0, common_1.Module)({
        imports: [
            schedule_1.ScheduleModule.forRoot(),
            order_module_1.OrderModule,
            mongoose_1.MongooseModule.forFeature([
                { name: 'Order', schema: order_schema_1.OrderSchema },
                { name: 'Block', schema: block_schema_1.BlockSchema },
            ]),
        ],
        providers: [
            contractEventSubscribe_service_1.ContractEventSubscribeService,
            ether_provider_1.EtherProvider,
            order_service_1.OrderService,
            seaport_provider_1.SeaportProvider,
            block_service_1.BlockService,
            MutexManager_1.MutexManager,
        ],
        exports: [contractEventSubscribe_service_1.ContractEventSubscribeService, MutexManager_1.MutexManager],
    })
], SubscribeModule);
//# sourceMappingURL=subscribe.module.js.map